...
var triggerLnk6 = document.getElementsByClassName('trigger-link-overlay');
...
function toggleOverlay(capa) {
	closeOpenOverlays();
	if( classie.has( capa, 'abre' ) ){ closeOverlay(capa); }
	else if( !classie.has( capa, 'cierra' ) ) { classie.add( capa, 'abre' ); }
}

function addNewListeners(){ $('#socios').on('click', "a.trigger-link-overlay", function(){ toggleOverlay(overlay4);});}
